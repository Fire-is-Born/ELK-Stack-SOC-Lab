...
## C2 Investigation and Event Correlation

With the initial compromise already investigated, I moved on to looking at the **Mythic C2 activity** on the Windows machine.

Since I already knew the name of the payload used during the attack simulation, I started by searching for:

```text
svchost-MyDFIR.exe
```

I then sorted the results from oldest to newest so I could start building a timeline of the C2 activity.

Of course, in a real investigation I probably wouldn't already know the name of the malicious file, so there are a few other ways I could have found this activity.

---

## Finding C2 Activity Without Knowing the Filename

One place to start would be the network telemetry.

An established C2 session will often generate repeated traffic between the compromised machine and the C2 server. Depending on what the attacker is doing, this could also involve a large amount of data being transferred and potentially cause the host or destination to appear among the **top talkers** on the network.

Another thing to look for is **beaconing**. C2 agents will often contact their server at regular intervals to check for new commands. A tool such as **RITA (Real Intelligence Threat Analytics)** can be useful for identifying this sort of behaviour.

Since I have Sysmon installed on the endpoint, I can also correlate **process creation events (Event ID 1)** with **network connections (Event ID 3)**. This means I can see not only that a connection happened, but also which process was responsible for it.

---

## Looking at Process Initiated Network Connections

I opened my `MyDFIR-Suspicious-Activity` dashboard and looked at the **Process Initiated Network Connections** panel.

<img width="2555" height="1123" alt="image" src="https://github.com/user-attachments/assets/470339fe-d75b-4d45-9e08-990add31d244" />
...
I already know this is the Mythic Apollo agent because I created the activity myself. However, even if the file had been given a less obvious name such as `update.exe`, I would still want to investigate it.
...
and making outbound network connections is unusual enough to question why it is there and what it is communicating with.

I also noticed another interesting entry:

```text
C:\Windows\System32\WindowsPowerShell\v1.0\powershell.exe
```

PowerShell was making an outbound connection to **port 9999**.

PowerShell itself is obviously legitimate, but it is commonly abused by attackers. Seeing it making an outbound connection, especially to an unusual port, makes it worth investigating further.

---

## Investigating the PowerShell Connection

I decided to start with the destination IP associated with the PowerShell connection:

```text
78.141.196.90
```

I searched for Sysmon network connection events involving this IP using:

```text
event.code: 3 and winlog.event_data.DestinationIp: 78.141.196.90
```

<img width="2252" height="279" alt="image" src="https://github.com/user-attachments/assets/0fcf5e56-55b8-4906-9629-799c8b7c6fd5" />

This returned three network connections:

| Time | Activity |
| --- | --- |
| Jul 20, 2026 @ 12:19:45 | Network connection to `78.141.196.90` |
| Jul 20, 2026 @ 12:24:30 | Network connection to `78.141.196.90` |
| Jul 20, 2026 @ 12:34:40 | Network connection to `78.141.196.90` |

The repeated connections to the same external IP give me another reason to investigate the process responsible for them.

---

## Correlating the PowerShell Activity

Since I have Sysmon telemetry available, I can use the **ProcessGuid** to correlate events generated by the same process.

<img width="2267" height="1138" alt="image" src="https://github.com/user-attachments/assets/e8c0fa25-cdc6-4846-b0b4-d8343509c061" />

Looking at the oldest relevant event, I found the following Process GUID:

```text
{69912F94-029B-6A5E-0301-000000000A00}
```

Rather than relying on the PID alone, the Process GUID is useful because Sysmon generates a unique GUID for each process. PIDs can eventually be reused by Windows, whereas the Process GUID lets me follow this specific PowerShell session.
...
This returned **19 events** associated with the PowerShell process.

---

## How PowerShell Was Started

Looking at the oldest process creation event showed that PowerShell had been opened by the `Administrator` user.
...
The parent process being `explorer.exe` suggests that PowerShell was launched interactively from the Windows desktop rather than being spawned directly by another suspicious process.

From here I could continue following the events associated with the PowerShell Process GUID.

---

## Finding the C2 Payload Being Written to Disk

One of the events associated with this PowerShell session was a **Sysmon Event ID 11 - File Create** event.

At:

```text
Jul 20, 2026 @ 12:12:31
```
...
This gives me a useful link between the PowerShell activity and the C2 payload.

The important information from the event was:
...
So at this point I can see that the PowerShell session wrote an executable into the Public Downloads directory.

---

## Following the New Process

I also found a process creation event for the payload.

The new process had the Process GUID:

```text
{69912F94-053F-6A5E-0601-000000000A00}
```

I searched for this GUID to see what other events were associated with the process.

I also tried searching using the parent PID to see if it had created any additional child processes.

Neither search returned anything particularly useful beyond what I had already found.

This doesn't necessarily mean the process did nothing else. It just means there weren't any additional relevant events available in the telemetry I had collected.

---

## C2 Payload Details

From the investigation I was able to identify the following information about the payload:
...
The important events I had identified so far were:

```text
Jul 20, 2026 @ 12:12:31
Event ID 11 - svchost-MyDFIR.exe written to disk by PowerShell

Jul 20, 2026 @ 12:19:45
Event ID 3 - Network connection to 78.141.196.90

Jul 20, 2026 @ 12:23:43
Event ID 1 - svchost-MyDFIR.exe process created

Jul 20, 2026 @ 12:24:30
Event ID 3 - Network connection to 78.141.196.90

Jul 20, 2026 @ 12:34:40
Event ID 3 - Network connection to 78.141.196.90
```

This starts to give me a much clearer picture of what happened on the endpoint.

---

## Looking for `passwords.txt`

During the original attack simulation I used Mythic to access and exfiltrate a file called:

```text
passwords.txt
```

Since I already knew this had happened, I searched the logs for the filename to see what endpoint evidence had been collected.

<img width="1725" height="517" alt="image" src="https://github.com/user-attachments/assets/843d4bd4-960b-48db-8555-978d5a547122" />

I found events showing the text file being accessed through Explorer and opened in Notepad.

This confirms activity involving the file on the endpoint, but I couldn't see the actual Mythic exfiltration of the file.

That makes sense because the actual transfer would be better observed through **network telemetry** rather than normal endpoint process and file events.

For example, having additional data from packet captures, network flow logs, a proxy or another network monitoring tool could potentially provide more visibility into what was being transferred between the endpoint and the C2 server.

This lab is mainly collecting endpoint telemetry, so there is a limit to what I can see.

It is a good example of why having both **endpoint and network telemetry** is useful during an investigation.

---

## Building a Timeline

At this point I could build a basic timeline of the C2 activity:

| Timestamp | Event | Finding |
| --- | --- | --- |
| Jul 20, 2026 @ 12:12:31 | Sysmon Event ID 11 | `svchost-MyDFIR.exe` written to the Public Downloads directory by PowerShell |
| Jul 20, 2026 @ 12:19:45 | Sysmon Event ID 3 | Connection made to `78.141.196.90` |
| Jul 20, 2026 @ 12:23:43 | Sysmon Event ID 1 | `svchost-MyDFIR.exe` process created |
| Jul 20, 2026 @ 12:24:30 | Sysmon Event ID 3 | Another connection to `78.141.196.90` |
| Jul 20, 2026 @ 12:34:40 | Sysmon Event ID 3 | Further connection to `78.141.196.90` |

The activity can roughly be followed as:

```text
PowerShell launched
        ↓
Executable written to Public Downloads
        ↓
Suspicious external network activity
        ↓
svchost-MyDFIR.exe executed
        ↓
Further communication with external infrastructure
```

Looking at the events together is much more useful than looking at each one individually.

For example, PowerShell making a network connection isn't automatically malicious. An executable in a Downloads directory isn't automatically malicious either.

But when I can see PowerShell writing an executable to disk, that executable being executed, and suspicious external network connections occurring around the same time, the activity becomes much more interesting.

---

## Testing the C2 Detection Rule

I already had an Elastic detection rule called:

```text
MyDFIR-Mythic-C2-Apollo-Agent-Detected
```

I wanted to take this a step further and have the rule automatically generate a ticket in the osTicket server I configured earlier in the project.

I edited the rule and configured it to use my webhook connector so that when the detection fires, Elastic sends the alert information to osTicket.

The original Mythic callback had expired, so I needed to recreate the activity to test the detection.

On the Windows server I renamed the payload to:

```text
mydfir.exe
```

I then downloaded it again using PowerShell and ran it as Administrator.

---

## Elastic Alert

The activity successfully triggered my detection rule.

<img width="2289" height="671" alt="image" src="https://github.com/user-attachments/assets/4944eccb-e763-4a2c-9a68-ad44d5b9a24b" />

Elastic generated a **Critical** alert:
...
This confirmed that the detection rule was still able to identify the Apollo agent after I recreated the activity.

---

## Automatic osTicket Creation

The webhook action configured on the rule also worked.

When Elastic generated the alert, a new ticket was automatically created in osTicket with the subject:

<img width="964" height="309" alt="image" src="https://github.com/user-attachments/assets/70dc460b-19f6-4eec-8609-5be51c64a1f4" />

```text
MyDFIR-Mythic-C2-Apollo-Agent-Detected
```

The ticket showed the source as:

```text
Elastic
```

I now have multiple detections automatically feeding into the ticketing system, including:

```text
MyDFIR-Mythic-C2-Apollo-Agent-Detected
MyDFIR-RDP-Brute-Force-Attempt
```

This means the workflow is no longer just generating alerts inside Elastic. Alerts can now automatically enter the ticketing system where an analyst could pick them up, investigate them and track their status.

The overall workflow now looks like:

```text
Suspicious activity occurs
        ↓
Sysmon records the activity
        ↓
Logs are ingested into Elastic
        ↓
Elastic detection rule triggers
        ↓
Security alert is generated
        ↓
Webhook sends the alert to osTicket
        ↓
Ticket is automatically created
        ↓
Analyst can begin the investigation
```
...
The biggest thing from this investigation was seeing how useful **event correlation** is.

Searching for a known malicious filename is easy, but that isn't normally how an investigation starts. Looking at unusual network connections and then working backwards to the process responsible gives me a much more realistic way of finding suspicious activity.

Sysmon's `ProcessGuid` was especially useful here. Once I had identified the suspicious PowerShell process, I could use its GUID to pull together the other events generated by that same process and start reconstructing what it had done.

Some of the things that would have caught my attention even without knowing about the Mythic attack beforehand were:

- An executable running from `C:\Users\Public\Downloads\`
- That executable making outbound network connections
- PowerShell making an outbound connection to an unusual port
- Repeated connections to the same external IP
- PowerShell writing an executable to disk

None of these necessarily proves malicious activity by itself, but together they give me enough reason to investigate further.

It also highlighted a limitation in my current lab. I have good endpoint visibility through Sysmon and Elastic, but much less visibility into the actual contents of network traffic.

I could see the processes involved, the file being created and the network connections taking place, but I couldn't directly see the `passwords.txt` file being transferred through the Mythic C2 channel.
